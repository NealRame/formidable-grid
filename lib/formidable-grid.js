...
function make_validator(validator_name, filters) {
    if (_.isArray(filters) && ! _.isEmpty(filters)) {
        var validators = _.map(filters, make_validator.bind(null, validator_name));
        return function(str) {
    ...
    if (_.isRegExp(filters)) {
        return function(str) {
            debug(util.format('-- validate %s %s', validator_name, str));
...
function handle_file_part(field_name, file, file_name, encoding, mime_type) {
    ...
    if (this.acceptFieldName(field_name) && this.acceptMimeType(mime_type)) {
        ...
        debug(util.format('-- create file: %s'), id.toString());

        promise = new Promise(function(resolve, reject) {
            ostream
                .once('end', function() {
                    debug(util.format('-- end of %s', id.toString()));
...
function handle_field_part(field_name, value) {
    var promise;
    debug('-- handle field part', field_name, util.inspect(value));
